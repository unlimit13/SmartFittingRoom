...
def _make_fake_result_gendered(tmp_path):
    """male/female 각각 1개 snap을 가진 가짜 musinsa_out 디렉토리를 생성한다."""
    snaps_by_gender = [("male", "snap_m", ["001", "002", "003"]),
                       ("female", "snap_w", ["004", "005", "006"])]
    ...
    result = []
    for gender, snap_id, goods_nos in snaps_by_gender:
...
def test_metadata_contains_all_products(converted):
    ...
    assert len(meta) == 6  # 3 male + 3 female
