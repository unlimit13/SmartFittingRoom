...
#
#   bash parallel/run_tp_pair.sh <steps> <outdir>
set -euo pipefail
STEPS="${1:-1}"
OUT="${2:-output/tp}"
...
SHARED="--tp --world_size 2 --master_addr 192.168.100.1 --master_port 29500 \
...
SSH_PID=$!
echo "[run_tp_pair] rank1 launched on $PEER (ssh pid $SSH_PID, steps=$STEPS)"

# rank 0 here (foreground, writer) -- runs concurrently with rank 1
echo "[run_tp_pair] rank0 start: $(date '+%T')"
eval "${ENV} .venv/bin/python inference_lowmem.py --rank 0 ${SHARED} \
  --checkpoint_path $RANK0_DIR/checkpoint --data_dir $RANK0_DIR/$DATA \
  --output_dir $OUT"
echo "[run_tp_pair] rank0 end: $(date '+%T')"
wait "$SSH_PID"; echo "[run_tp_pair] rank1 (ssh) exited rc=$?"
