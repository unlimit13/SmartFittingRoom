"""
Convert musinsa_out_men / musinsa_out_women result.json to data/musinsa_db/ format.
Snap-based outfit data → flat product catalog + snap_outfits.json.
Each product entry includes a "gender" field ("남" or "여").
"""
import json
import os
import shutil
...
# (source_folder_name, gender_label)
SOURCES = [
    (os.path.join(ROOT, "musinsa_out_men"), "남"),
    (os.path.join(ROOT, "musinsa_out_women"), "여"),
]
...
def main():
    ...
    for src_dir, gender in SOURCES:
        result_json = os.path.join(src_dir, "result.json")
        with open(result_json, encoding="utf-8") as f:
            snaps = json.load(f)

        snap_prefix = "men" if gender == "남" else "women"
